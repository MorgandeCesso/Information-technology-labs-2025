...
Для входов использовались профили root (в Gitlab) и admin (в Nexus). Пароли для них хранятся внутри самих контейнеров и их можно просмотреть через ```docker exec -it "имя контейнера" cat ```. Временный пароль для Gitlab находится в контейнере по пути: /etc/gitlab/initial_root_password, а пароль для Nexus - /nexus-data/log/nexus.log. Временный пароль Gitlab хранится сутки, после чего удаляется, а у Nexus пароль удаляется сразу после первого входа.


После ввода пароля в каждом из сервисов их необходимо поменять. В Gitlab это делается так:

1. Заходим в терминал Gitlab: ```docker exec -it gitlab /bin/bash```;
2. Заходим в консоль rails Gitlab'a: ```gitlab-rails console -e production```;
3. Меняем пароль при помощи команд:

```
user = User.find_by(username: 'root')
user.password = 'новый_пароль'
user.password_confirmation = 'новый_пароль'
user.save!
```

5. Перезапускаем сервис: ```docker compose restart gitlab```;
6. Перезаходим в root с новым паролем.

Для Nexus'a пароль меняется в интерфейсе контейнера.
...
_Примечание:_ Поскольку в файл по пути ```/etc/docker/daemon.json``` добавить insecure registry в дальнейшем не получилось, было решено прописать его для docker-dind через параметры запуска, что и сработало.
...
2. Даем разрешение на запуск gitlab-runner из директории:
...
6. При регистрации задаются вопросы, самое главное: указываем токен, адрес репозитория на хосте и executor'а (исполнителя) как Docker.
...
[[runners]]
  ...
  id = 3
  token = "glrt-IKt6wdCY7dijAnJPiEoXDm86MQpwOjEKdDozCnU6MQ8.01.1714e2244"
  ...
  [runners.docker]
    ...
    run_untagged = true
    volumes = ["/cache"]
    extra_hosts = ["gitlab.local:10.222.57.237", "nexus.local:10.222.57.237"]
    network_mtu = 0
~~~
